...
6. **Forest packing** (`layoutForest`) — block order is shared → personal by creation order with Unassigned last, never by size; the unassigned grid wraps at the viewport width ordered by system id; an empty chain yields a 0×0 block; `blockGap` spaces the shelf packing without touching intra-tree gaps.
