...
- Opens/toggles on ⌘K / Ctrl-K or bare `/` via a document-level keydown listener that ignores editable targets and anything inside an open dialog (`SignaturePasteHotkey` idiom) — an input's own ⌘K and `/` stay native.
