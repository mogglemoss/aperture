...
Auth: `requireSystemNoteMutate(session)` — any authenticated character (401 if not signed in). Body (Zod): `systemId` int>0, `body` 1–2000. Calls `createSystemNote({ ...body, characterId })` (which also writes a `create` audit event), then `withAuthorName(row)`.
