...
Auth: `requireSystemNoteMutate(session)` — any authenticated character (401 if not signed in). Body (Zod): `systemId` int>0, `body` 1–2000, `category` enum (`intel`/`journal`/`bounty`/`logistics`/`warning`) nullable optional, `locked` boolean optional. Calls `createSystemNote({ ...body, characterId })` (which also writes a `create` audit event), then `withAuthorName(row)`.
