...
Auth: `requireSystemNoteMutate(session)` — any authenticated character (401 if not signed in). Body (Zod): `systemId` int>0, `body` 1–2000, `category` nullable optional (validated against the `apertureConfig.SYSTEM_NOTE_CATEGORIES` keys), `locked` boolean optional. Calls `createSystemNote({ ...body, characterId })` (which also writes a `create` audit event), then `withAuthorName(row)`.
