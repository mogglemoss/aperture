...
Auth: `requireSystemNoteMutate(session)` — any authenticated character. Body (Zod): any of `body` 1–2000, `category` enum nullable, `locked` boolean; at least one key required. Calls `updateSystemNote` (which also writes an `update` audit event), then `withAuthorName(row)`.
